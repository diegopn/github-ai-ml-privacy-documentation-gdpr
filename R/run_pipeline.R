...
source(file.path("R", "select_sample.R"))
...
parse_pipeline_options <- function(args) {
  values <- list(input = file.path("data", "repositorios_selecionados.csv"), workers = 1L, output = "outputs", select = FALSE)
  index <- 1L
  while (index <= length(args)) {
    ...
    if (identical(option, "--select")) {
      ...
      index <- index + 2L
    } else stop("Uso: Rscript R/run_pipeline.R [--select] [--input arquivo.csv] [--workers N] [--output diretório]")
  }
  values
}
